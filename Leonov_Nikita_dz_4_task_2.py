...
def currency_rates(char_code):
    ...
    for i in ask_content:  # Похоже я сейчас изобретаю велосипед но интересно что получется
        i = i.split('/')
        ...
        if char_code == test_char_code:
            print(f' {test_nominal} ед. {char_code} ровна {test_value} рублям на момент запроса')


currency_rates('JPY')
currency_rates('USD')
currency_rates('EUR')
currency_rates('OOJ')  # Не могу понять как вернуть None если такой валюты нет :(